...
let std_connect ~broker ~port ~proxy =
  ...
  let%lwt socket =
    if proxy_host <> ""
    then connect proxy_host proxy_port broker port
    else Mqtt_lwt.connect_socket ~host:broker ~port
  in
...
let start_client ~id ~broker ~port ~proxy ~certs =
  ...
  let%lwt conn =
    if valid certs
    then tls_connect ~broker ~port ~proxy ~certs
    else std_connect ~broker ~port ~proxy
  in
