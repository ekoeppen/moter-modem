...
let rec wait_for_packet ic =
  ...
  if c1 != dle || c2 != stx then wait_for_packet ic else Lwt.return ()
;;

let read_escaped_char ic =
  let%lwt c = Lwt_io.read_char ic in
  if c == dle then Lwt_io.read_char ic else Lwt.return c
;;
...
let lwt_main device =
  ...
  output_string oc "\x10\x02Hello, World!\x10\x03";
  flush oc;
...
let test_lwt _ device = Lwt_main.run (lwt_main device)
...
let cmd =
  ...
  Term.(const test_lwt $ logging $ device), Term.info "serial_test" ~doc ~exits
;;
